...
class ArrayConstructorLowering(val context: CommonBackendContext) : IrElementTransformerVoidWithContext(), FileLoweringPass {
    ...
    // TArray(size) or unbound arrayOfNulls<T>(size).
    private val IrClassSymbol.sizeConstructor: IrFunctionSymbol
        get() = constructors.find { it.owner.valueParameters.size == 1 } ?: context.ir.symbols.arrayOfNulls

    // TArray(size, function) or unbound Array<T>(size, function).
    private val IrClassSymbol.inlineConstructor: IrFunctionSymbol
        get() = constructors.single { it.owner.valueParameters.size == 2 }

    private val arrayInlineToSizeCtor =
        (context.irBuiltIns.primitiveArrays + context.irBuiltIns.arrayClass)
            .associate { it.inlineConstructor to it.sizeConstructor }
